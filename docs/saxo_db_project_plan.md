...
作成日: 2026-07-16  
状態: **BOOTSTRAP COMPLETE / DATABASE NOT BUILT**

## 1. 目的

`saxo_api`の責務を、Saxo認証・データ取得・既存研究の監査元に限定する。データの永続管理、増分更新、品質履歴、研究snapshot、分析query、backup/restoreは独立した`saxo_db`で実施する。
...
コピーした旧仕様は監査原本として編集しない。新しい相対pathとimport区分は`specs/saxo_db_import_spec.json`で上書きせずに追加定義する。
...
2. instrument masterを`catalog`へ登録
3. ingestion runとsource fileを`ops`へ登録
...
- Docker container・volume・database作成
- migration・dependency install
- DB import
- Saxo API呼出し
- token・口座情報保存
- strategy signal、PnL、WFO、portfolio計算

次に許可される実装作業はDB1だが、利用者の明示指示があるまで開始しない。
