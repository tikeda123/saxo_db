...
BEGIN
    IF current_database() <> 'saxo_market' THEN
        RAISE EXCEPTION '0022_s6v5a_calendar_view_performance.sql applied to unexpected database %', current_database();
    END IF;
END
$$;

-- Preserve the complete-slot semantics introduced by 0021 while avoiding a
-- per-row interval range join and a full historical generate_series. The
-- equality join on session_date keeps series-status below the Read API's
-- statement timeout as the canonical history grows.
    ...
    JOIN catalog.session_interval si
     ...
     AND si.session_date = CASE
       WHEN i.asset_type='FxSpot'
         THEN (timezone(c.timezone_name,b.time_utc)-interval '17 hours')::DATE
