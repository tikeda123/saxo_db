...
BEGIN
    IF current_database() <> 'saxo_market' THEN
        RAISE EXCEPTION '0020_s6v5a_complete_slot_freshness.sql applied to unexpected database %', current_database();
    END IF;
END
$$;
...
WITH actual AS (
    ...
    CROSS JOIN LATERAL generate_series(
        CASE
            WHEN a.asset_type = 'FxSpot'
                THEN date_trunc('hour', si.open_time_utc) + interval '1 hour'
            ELSE si.open_time_utc
        END,
        CASE
            WHEN a.asset_type = 'FxSpot'
                THEN date_trunc('hour', si.close_time_utc) - interval '1 hour'
            ELSE si.close_time_utc - interval '1 hour'
        END,
        interval '60 minutes'
    ) AS slot(time_utc)
    ...
    JOIN catalog.session_interval si
     ...
     AND (
         (i.asset_type = 'FxSpot' AND b.time_utc = date_trunc('hour', b.time_utc))
         OR
         (i.asset_type <> 'FxSpot'
